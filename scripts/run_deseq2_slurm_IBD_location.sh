...
/nfs/data/COST_IBD/downstream_tasks/dea/scripts/deseq2.R \
    ...
    --category_column "Disease.status" \
    --cell_type_column "annotation.large" \
    --sample_column sample \
    --count_assay X \
    --split_column "Disease.location" \
    --split_groups "Terminal Ileum" "Colon" "Left Colon" "Sigmoid Colon" "Small Bowel" "Right Colon" "Rectum" "Right Colonlon" "Cecum" "Transverse Colon" \
    --remove_groups 'Healthy (Deceased donation or Routine Screenings)' 'Healthy (Unknown)' 'Healthy (Altered bowel movements)' 'Healthy (Cancer patient)' \
    --output_directory /nfs/data/COST_IBD/downstream_tasks/dea/output/ibd_region_inflammation \
    --remove_cell_type 'NA'
